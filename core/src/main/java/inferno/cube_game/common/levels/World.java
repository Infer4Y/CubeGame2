...
public class World {
    ...
    public World() {
        ...
        chunkGeneratorExecutor = Executors.newFixedThreadPool(3); // Create a thread pool for chunk generation
    }
    ...
    public Chunk getChunk(int chunkX, int chunkY, int chunkZ) {
        ...
        // Attempt to load chunk from file if not in memory
        Chunk loadedChunk = loadChunk(chunkX, chunkY, chunkZ);
        if (loadedChunk != null) {
            loadingChunks.put(key, CompletableFuture.completedFuture(loadedChunk)); // Store loaded chunk in map
            return loadedChunk;
        }
    ...
    private void cullTooFarChunks(int playerChunkX, int playerChunkY, int playerChunkZ) {
        ...
        loadingChunks.entrySet().removeIf(entry -> {
            ...
            saveChunk(entry.getValue().resultNow());
    ...
    private static final String CHUNK_DIRECTORY = "world/chunks/";

    public static Chunk loadChunk(int chunkX, int chunkY, int chunkZ) {
        String fileName = "chunk_" + chunkX + "_" + chunkY + "_" + chunkZ + ".chunk";
        FileHandle fileHandle = Gdx.files.local(CHUNK_DIRECTORY + fileName);

        if (!fileHandle.exists()) {
            Gdx.app.log("ChunkIO", "Chunk file not found: " + fileHandle.path());
            return null;
        }

        try (InputStream fileIn = fileHandle.read();
             DataInputStream in = new DataInputStream(fileIn)) {

            // Read saved chunk coordinates
            int loadedChunkX = in.readInt();
            int loadedChunkY = in.readInt();
            int loadedChunkZ = in.readInt();

            // Read palette size
            int paletteSize = in.readInt();
            LinkedHashMap<Byte, Block> palette = new LinkedHashMap<>(paletteSize);

            // Read the palette data
            for (int i = 0; i < paletteSize; i++) {
                byte paletteIndex = in.readByte();
                Block block = BlockRegistry.getBlock(Arrays.toString(in.readNBytes(in.readInt())));
                palette.put(paletteIndex, block);
            }

            // Read block palette indices
            byte[] blockPaletteIndices = new byte[Chunk.CHUNK_SIZE * Chunk.CHUNK_SIZE * Chunk.CHUNK_SIZE];
            in.readFully(blockPaletteIndices);

            // Return new Chunk using the secondary constructor
            return new Chunk(loadedChunkX, loadedChunkY, loadedChunkZ, palette, blockPaletteIndices);
        } catch (IOException e) {
            Gdx.app.error("ChunkIO", "Error loading chunk from disk: " + fileHandle.path(), e);
        }
        return null; // Return null if loading fails
    }

    public static void saveChunk(Chunk chunk) {
        String fileName = "chunk_" + chunk.getChunkX() + "_" + chunk.getChunkY() + "_" + chunk.getChunkZ() + ".chunk";
        FileHandle fileHandle = Gdx.files.local(CHUNK_DIRECTORY + fileName);

        try (OutputStream fileOut = fileHandle.write(false)) {
            ...
            // Save palette size
            out.writeInt(chunk.getPalette().size()); // Assuming a method to get palette size

            // Save the palette data
            for (Byte paletteIndex : chunk.getPalette().keySet()) {
                out.writeByte(paletteIndex);
                Block block = chunk.getPalette().get(paletteIndex);
                String blockRegisteredName = block.getDomain() + block.getRegistryName();
                out.writeInt(blockRegisteredName.length());
                out.writeBytes(blockRegisteredName); // Save block ID or similar representation
            }

            // Save block palette indices
            out.write(chunk.getBlocks());
        } catch (IOException e) {
            Gdx.app.error("ChunkIO", "Error saving chunk to disk: " + fileHandle.path(), e);
        }
    }
